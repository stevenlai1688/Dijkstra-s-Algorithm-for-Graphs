...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected class Path implements Comparable<Path> {
    ...
    public Path(Path copyPath, Edge extendBy) {
      ...
      // copy the path passed to it, and extend it by a single edge passed as the second parameter
      // The final node of the new path will be the target vertex of this edge
      this.start = copyPath.start;
      this.end = extendBy.target;
      // to update the path's overall costs
      this.distance = extendBy.weight + copyPath.distance;
      // making sure that dataSequence field in the newly constructed instance references a
      // different list object
      // copy the data sequence before
      this.dataSequence = new LinkedList<>();
      // loop through the copyPath's dataSequence, and add all previous data sequence to this new
      // dataSequence linkedlist
      for (int i = 0; i < copyPath.dataSequence.size(); i++) {
        this.dataSequence.add(copyPath.dataSequence.get(i));
      }
      // then add the new data sequence and add the extendBy vertex
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    // create a priority queue that holds all the path yet to discover
    PriorityQueue<Path> discoverablePath = new PriorityQueue<Path>();
    // create a priority queue that holds all the visited vertices
    PriorityQueue<T> visitedVertex = new PriorityQueue<T>();
    // create another priority queue that holds all the correct path based on the lowest cost >
    // biggest cost, and to choose the shortest path, simply return the head of the queue.
    PriorityQueue<Path> correctPaths = new PriorityQueue<Path>();
    // double check that both vertices exist
    if (vertices.get(start) == null || vertices.get(end) == null) {
      throw new NoSuchElementException("No Path found");
    }
    // create a new path based off of the selected starting vertex
    Path startingPath = new Path(vertices.get(start));
    // make a ending target to be matched with
    Vertex endingTarget = vertices.get(end);
    // add the starting vertex to the priority queue
    discoverablePath.add(startingPath);

    // while the priority queue is not empty, which will go through all possible path, popping out
    // the first node to be checked, then adding neighbor nodes to extend to the new path to add
    // into the priority queue again, thus looping until all paths are checked
    while (discoverablePath.size() != 0) {

      // pop the first node visited from the priority queue
      // we will continue to use this popped path to determine the next paths to add on
      Path popped = discoverablePath.poll();
      // use the currentPath's ending vertex to determine the current vertex
      Vertex currentVertex = vertices.get(popped.end.data);

      // create a new linkedList of neighborEdges and iterate through the linkedList
      LinkedList<CS400Graph<T>.Edge> neighborEdges = currentVertex.edgesLeaving;

      // go through the list of edges leaving the currentPath to find neighboring vertexes
      for (int j = 0; j < neighborEdges.size(); j++) {
        // if the node has not been visited already
        if (!visitedVertex.contains(neighborEdges.get(j).target.data)) {
          // create a new neighbor vertex object that is an extension of current path (popped path
          // from discoverable path) + the edges
          Path neighbor = new Path(popped, neighborEdges.get(j));
          // add the visited vertex which is what popped from the discoverablePath priority queue
          // after we found the path, first making sure that the vertex found is not null
          if (popped.end != null) {
            visitedVertex.add(popped.end.data);
          }
          // add the neighbor to the priority queue, the neighbor is going to be in the path which
          // has lowest cost, determined by its position in the priority queue
          discoverablePath.add(neighbor);
        }
      }
      // ending condition:
      // if popped path finds the ending target at its ending vertex,
      if (popped.end != null && popped.end.equals(endingTarget)) {
        // add the path that reached the end to compare later
        correctPaths.add(popped);
      }
    }

    // if Search does not fail to find any path from the start to end vertices
    if (correctPaths != null && correctPaths.size() != 0) {
      // finally check to use to determine the shortest path
      // since correctPaths is a priority queue, its head will be the shortest path
      // find the path with the lowest weight and return it
      return correctPaths.peek();
    } else {
      // if it does not return a correctPath
      throw new NoSuchElementException("No Path found");
    }
  }
